...
const socialItems = [
  {
    ...
    icon: WhatsAppIcon,
    color: "#25D366",
    getHref: (value: string) => `https://wa.me/${value.replace(/[^0-9+]/g, "")}`,
    label: "WhatsApp",
  },
  {
  ...
  },
  {
    ...
    icon: PhoneIcon,
    color: "#6B7280",
    getHref: (value: string) => `tel:${value.replace(/[^0-9+]/g, "")}`,
    label: "Phone",
  },
];
...
export function FloatingSocial({
  ...
  const activeItems = socialItems.filter((item) => {
    const fieldKey = dataKeyMap[item.key];
    ...
    <div className="fixed bottom-6 right-6 z-40 flex flex-col-reverse items-center gap-3">
      ...
      >
        <svg
          ...
          className={`w-6 h-6 transition-transform duration-300 ${isOpen ? "rotate-135" : ""}`}
        >
          <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z" />
        </svg>
      </button>
      ...
      {activeItems.map((item, index) => {
        ...
        return (
          <a
            ...
            className="w-11 h-11 rounded-full shadow-lg flex items-center justify-center text-white transition-all duration-300 hover:scale-110"
            style={{
